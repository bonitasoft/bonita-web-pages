...
class ProcessList extends Component {
  ...
  fetchPage(page = 0, _filters) {
    ...
    ProcessApi.fetchPage({ ...pagination, page }, { ...filters, ..._filters })
              .then(({ unpopulated, populated }) => {
                unpopulated.then(({ processes, pagination }) => this.setState({ processes, pagination }));
  ...
  render() {
    ...
    return (
      <div className="ProcessList container border">
        <h1>Processes</h1>
        <Filters filters={filters} categories={categories} onChange={this.updateFilters} />
        <List processes={processes} pagination={pagination} filters={filters} toggleOrder={this.toggleOrder} />
